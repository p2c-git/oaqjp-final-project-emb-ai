'''Server to deploy emotion detector app
'''
...
def sent_detector():
    ''' This code receives the text from the HTML interface and 
        ...
        score for the provided text along with dominant emotion.
    '''
    text_to_analyze = request.args.get('textToAnalyze')
    response = emotion_detector(text_to_analyze)

    if response['dominant_emotion'] is None:
        response_display = 'Invalid text! Please try again!'
    else:
        response_display = f"For the given statement, the system response is \
